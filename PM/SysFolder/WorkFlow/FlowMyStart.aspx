<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FlowMyStart.aspx.cs" Inherits="EIS.Web.SysFolder.WorkFlow.FlowMyStart" %>
...
<!--
    var _curClass = EIS.Web.SysFolder.WorkFlow.FlowMyStart;
			        ...
			        if ($('.trSelected', grid).length > 0) {
			            if (confirm('确定删除这' + $('.trSelected', grid).length + '条记录吗?')) {
			                $('.trSelected', grid).each
                                    (
                                        function () {
                                            var ret = EIS.Web.SysFolder.WorkFlow.FlowMyStart.RemoveInstance(this.id.substr(3));
                                            if (ret.error) {
