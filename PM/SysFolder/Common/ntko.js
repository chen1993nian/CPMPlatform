//以下是程序引用页面的定义
var editMScode="neweditoffice.aspx";//编辑MS OFFICE文档页面程序
var editWPScode="editwps.aspx";//编辑WPS文档页面程序
var savecode="WebOfficeSave.aspx";//保存文档程序程序
...
{
    window.open(editMScode+"?newofficetype="+newofficetype+"&url="+url,"editoffice","top=0,left=0,toolbar=no,menubar=no,resizable=yes");
}
//当前表单不可编辑,当整个表单只读时调用
...
function editoffice(fname,newofficetype)
{
    ...
    if( (typeof(fname) != "undefined") && (fname != "") )
	{
	    ...
	    var url="FileDown.aspx?fileid="+filename+"&t="+Math.random();
        ...
        if (mburl != "") {
            var url = "FileDown.aspx?fileid=" + mburl + "&t=" + Math.random();
            try { TANGER_OCX_OBJ.BeginOpenFromURL(url); } catch (err) { };
            return;
        }
        switch(newofficetype)
...
function saveFileToUrl()
{
	...
	if(IsFileOpened)
	{
		...
		retHTML = TANGER_OCX_OBJ.saveToURL(savecode+"?filedot="+filedot,//提交到的url地址
		"fileid",//文件域的id，类似<input type=file id=upLoadFile 中的id
		"",
		"",
		0
		);
...
{
	var signUrl="FileDown.aspx?fileid="+fileId+"&t="+Math.random();
	if(IsFileOpened)
	{
			try
			{
...
function insertRedHeadFromUrl(fileId)
{
	...
	var headFileURL="FileDown.aspx?fileid="+fileId+"&t="+Math.random();
	//alert(headFileURL);
...
function openTemplateFileFromUrl(templateUrl,asyncLoad) {
    if(arguments.length == 1)
        asyncLoad=true;
    var url = "FileDown.aspx?fileid=" + templateUrl + "&t=" + Math.random();
    try { 
        if(asyncLoad){
