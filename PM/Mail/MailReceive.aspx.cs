...
{
	public partial class MailReceive : PageBase
	{
		...
		public void DeleteMail(string[] recIdList)
		{
			...
			{
				__MailReceiver.Delete(strArrays[i]);
			}
		}
		...
		[AjaxMethod]    // JustDecompile was unable to locate the assembly where attribute parameters types are defined. Generating parameters values is impossible.
		public void MoveMail(string[] mailIdArr, string targetFolderId)
		{
			_MailReceiver __MailReceiver = new _MailReceiver();
			string[] strArrays = mailIdArr;
			for (int i = 0; i < (int)strArrays.Length; i++)
			{
				__MailReceiver.UpdateFolder(strArrays[i], targetFolderId);
			}
		}

		protected void Page_Load(object sender, EventArgs e)
		{
			AjaxPro.Utility.RegisterTypeForAjax(typeof(MailReceive));
		...
		public void RemoveMail(string[] recIdList)
		{
			...
			for (int i = 0; i < (int)strArrays.Length; i++)
			{
				__MailReceiver.Remove(strArrays[i]);
			}
		}
	}
}