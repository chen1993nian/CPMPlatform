...
{
	public partial class MailTrash : PageBase
	{
		...
		public void MoveMail(string[] recIdList, string targetFolderId)
		{
			...
			for (int i = 0; i < (int)strArrays.Length; i++)
			{
				...
				if (strArrays1[0] != "")
				{
					...
					__MailReceiver.Delete(strArrays1[0], 0);
					__MailReceiver.UpdateFolder(strArrays1[0], targetFolderId);
				}
				else
				{
					_MailMessage __MailMessage = new _MailMessage();
					__MailMessage.Delete(strArrays1[1], 0);
					__MailMessage.UpdateFolder(strArrays1[1], targetFolderId);
				}
			}
		}
		...
		{
			AjaxPro.Utility.RegisterTypeForAjax(typeof(MailTrash));
			...
			{
				string str = "{{text:'{1}',func:function(){{menuClick('{0}');}}}},";
				this.sbMenu.AppendFormat(str, row["_autoId"], row["FolderName"]);
			}
			if (this.sbMenu.Length > 0)
			{
				this.sbMenu.Length = this.sbMenu.Length - 1;
			}
		}
		...
		public void RemoveMail(string[] recIdList)
		{
			...
			for (int i = 0; i < (int)strArrays.Length; i++)
			{
				...
				if (strArrays1[0] != "")
				{
					(new _MailReceiver()).Remove(strArrays1[0]);
				}
				else
				{
					(new _MailMessage()).Delete(strArrays1[1], 2);
				}
			}
		}
	}
