...
KE.lang = {
	...
	invalidHspace : "水平空白必须为数字。",
    invalidVspace : "垂直空白必须为数字。",
    invalidUrl : "URL不正确。",
    pleaseInput : "请输入内容"
};
...
KE.plugin['image'] = {
    ...
    check : function(id) {
		...
		var hspace = KE.$('imgHspace', dialogDoc).value;
		var vspace = KE.$('imgVspace', dialogDoc).value;
		var align = KE.$('imgAlign', dialogDoc).value;
		...
		if (hspace.match(/^\d+$/) == null) {
            alert(KE.lang['invalidHspace']);
            window.focus();
            KE.g[id].yesButton.focus();
            return false;
        }
		if (vspace.match(/^\d+$/) == null) {
            alert(KE.lang['invalidVspace']);
            window.focus();
            KE.g[id].yesButton.focus();
            return false;
        }
    ...
    exec : function(id) {
        ...
        if (type == 1) {
            ...
            this.insert(id, url, title, width, height, border,hspace,vspace,align);
        }
    },
    insert : function(id, url, title, width, height, border,hspace,vspace,align) {
        var html = '<img src="' + url + '" ';
        if (width > 0) html += 'width="' + width + '" ';
        if (height > 0) html += 'height="' + height + '" ';
		if (hspace > 0) html += 'hspace="' + hspace + '" ';
        if (vspace > 0) html += 'vspace="' + vspace + '" ';
