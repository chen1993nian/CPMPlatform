<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SurveyLeft.aspx.cs" Inherits="EIS.Web.WorkAsp.Survey.SurveyLeft" %>
...
<head id="Head1" runat="server">
	...
	<body>
		<form id="Form1" method="post" runat="server">
         <div class="toolbar" style="">
            ...
            <button id="btnAdd" type="button"><img alt="增加题目" src="../../img/common/add.png" />增加题目</button>
    ...
    <script type="text/javascript">
        ...
        {
            if(node.id){
                window.open("SurveyEdit.aspx?surveyId=<%=surveyId %>&nodeId=" + node.id + "&time=" + Math.random(), "main");
        }
    }
    ...
    var zNodes =<%=treedata %>;
        ...
        $(function () {
            ...
            root =zTree.getNodeByTId("tree_1");
            $("#btnAdd").click(function(){
                var nodeName="";
                if(root.children)
                {
                    nodeName="问题" + (root.children.length+1);
                }
                else
                {
                    nodeName="问题1";
                }
                var ret = _curClass.AddSonNode(nodeName,root.value);
                if(ret.error)
                {
                    alert("添加问题时出错："+ret.error.Message);
                }
                else
                {
        ...
        var _curClass = EIS.Web.WorkAsp.Survey.SurveyLeft;
        function addNode(retArr)//增加子节点
        {
            var son = { id: retArr[0], name: retArr[1], value: "" };
            zTree.addNodes(root, son);
        }
