...
function _enterSearch(ctl) {
    ...
    if (v.length == 0 && event.keyCode == 13) {
        ...
        cond.push("cid=" + arr[1]);
        cond.push("condition=" + scope);//escape(scope).replace(/\+/g, '%2B'));
        if (arr.length > 5) {
            cond.push("multi=" + arr[4]);
        }
        var para = cond.join("&");
        para = "para=" + _curClass.CryptPara(para).value;
        _openPage("../AppFrame/AppOutSelect.aspx?" + para);
    }
        ...
        if (dt.Rows.length == 1) {
            ...
            for (var i = 0; i < arrCond.length; i++) {
                initArr.push(arrCond[i] + " like '%" + v + "%'");
            }
            if (initArr.length > 0) {
                cond.push("initCond=" + initArr.join(" or "));
            }
            cond.push("condition=" + scope);
            if (arr.length > 5) {
                cond.push("multi=" + arr[4]);
            }
            var para = cond.join("&");
...
function _clickFrame(ctl) {
    ...
    var para = cond.join("&");
    para = "para=" + _curClass.CryptPara(para).value;
    _openCenter("../AppFrame/AppTreeSelFrame.aspx?" + para, '_blank', 900, 500);
}
	    ...
	    fldName = fldName.replace(/!/g,"");
            if (arguments.length == 3) {
                if (fldName.substring(0, 1) == ".") {
                    ...
                    source = source.replace(matches[i], _getCtlValueByFieldName(fldName));
                }
            }
            else {
                source = source.replace(matches[i], _getCtlValueByFieldName(fldName));
...
function _fnSubEdit(subName, rowId) {
    ...
    if (subOdr == -1) {
        ...
        var xmlEl = rowNode.find("_AutoID");
        var subId = xmlEl.text();
        var url = "../AppFrame/AppSub.aspx?tblName=" + subName + "&rowId=" + rowId + "&subId=" + subId + "&mainId=" + _mainId + "&sindex=" + _sIndex;
        var dlg = new jQuery.dialog({ title: '编辑', maxBtn: true, page: url
                    , btnBar: false, cover: true, lockScroll: true, width: 900, height: 600, bgcolor: 'black'
        });
        dlg.ShowDialog();
...
function _openPage(url) {
    ...
    var whht = $(window).height();
    if (window.screen) {
         whht = screen.availHeight - 140;
         wwth = screen.availWidth - 300;
    }
    _openCenter(url, "_blank", wwth, whht);
}
