...
(function() {
	...
	var phrases = {
		...
		'validationErrors': {
			'duplicate': '文件 <em>{name}</em> 已经存在, 不允许重复上传.',
			'sizeLimitMin': '文件 <em>{name}</em> (<em>{size}</em>) 太小, 至少 {fileSizeMin}.',
			'sizeLimitMax': '文件 <em>{name}</em> (<em>{size}</em>) 超过最大限额（ <em>{fileSizeMax}</em>）.',
			'fileListMax': '文件 <em>{name}</em> could not be added, amount of <em>{fileListMax} files</em> exceeded.',
			'fileListSizeMax': '文件 <em>{name}</em> (<em>{size}</em>) is too big, overall filesize of <em>{fileListSizeMax}</em> exceeded.'
		},
		'errors': {
			'httpStatus': '上传文件错误 #{code}',
