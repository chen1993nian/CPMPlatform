<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DefQueryStyleList.aspx.cs" Inherits="EIS.Studio.SysFolder.DefFrame.DefQueryStyleList" %>
...
<!--
    ...
    $("#flex1").flexigrid
			(
			{
				...
				{ display: '样式索引', name: 'styleindex', width: 60, sortable: true, align: 'center' },
				{ display: '样式名称', name: 'stylename', width: 200, sortable: true, align: 'left' },
            ...
            function sonTable(fldval, row) {
                ...
                var arr = [];
                arr.push("<a class='normal red' href='DefTableUI.aspx?tblName=<%=tblName %>&styleindex=" + fldval + "&t=1" + "'>列表界面</a>");
	...
	function app_add(cmd, grid) {
	    ...
	    var r = EIS.Studio.SysFolder.DefFrame.DefQueryStyleList.AddStyle("<%=tblName %>");
        if (r.error) {
        ...
        if ($('.trSelected', grid).length > 0) {
            if (confirm('确定删除这' + $('.trSelected', grid).length + '条记录吗?')) {
                $('.trSelected', grid).each
			            (
			                function () {
			                    var ret = EIS.Studio.SysFolder.DefFrame.DefQueryStyleList.DelRecord(this.id.substr(3));
			                    if (ret.error) {
