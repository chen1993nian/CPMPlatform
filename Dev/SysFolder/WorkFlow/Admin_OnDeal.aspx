                ...
                function app_finish(cmd, grid) {
                    if ($('.trSelected', grid).length > 0) {
                        if (confirm('您确定结束这个流程任务吗?')) {
                            ...
                            (
                                function () {
                                    var ret = EIS.WebBase.SysFolder.WorkFlow.Admin_OnDeal.FinishInstance(this.id.substr(3));
                                    if (ret.error) {
                ...
                function app_delete(cmd, grid) {
                    if ($('.trSelected', grid).length > 0) {
                        if (confirm('确定删除这' + $('.trSelected', grid).length + '条记录吗?')) {
                            ...
                            (
                                function () {
                                    var ret = EIS.WebBase.SysFolder.WorkFlow.Admin_OnDeal.RemoveInstance(this.id.substr(3));
                                    if (ret.error) {
                                        alert(ret.error.Message);
                                    }
                                }
