...
{
    public partial class FunLimitRole : AdminPageBase
    {
        ...
        public string GetListTree(string PIDValue, string Indent)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                object[] objArray = new object[] { "<tr name='tr", str2, "' id='tr", str2, "' style='", str7, "' state='", str6, "' sonnum=", length1, " ><td class='nodetd'>" };
        ...
        {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.AppendFormat("SELECT d._AutoID DeptId, d.RoleName DeptName, d._AutoID DeptWBS ,'0' DeptPWBS ,d.OrderId,isnull(m.FunID,'') FunID,isnull(m.FunLimit,'') Limit \r\n                 From T_E_Org_Role AS d LEFT OUTER JOIN T_E_Org_RoleLimit AS m ON d._AutoID = m.RoleID \r\n                 and m.FunID = '{0}' where d._isDel=0 ", string_1);
            return SysDatabase.ExecuteTable(stringBuilder.ToString());
        }

        protected void Page_Load(object sender, EventArgs e)
        {
            AjaxPro.Utility.RegisterTypeForAjax(typeof(FunLimitRole));
        ...
        public string SaveLimit(string funId, string limitxml)
        {
            ...
            try
            {
                ...
                foreach (XmlNode xmlNodes in xmlDocument.SelectNodes("//tn[@EditFlag='1']"))
                {
                    ...
                    {
                        strArrays = new string[] { "update dbo.T_E_Org_RoleLimit set FunLimit ='", value, "',funwbs='", funWbsById, "' where funid='", funId, "' and RoleId='", value1, "'" };
