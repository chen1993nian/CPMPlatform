...
{
    public partial class FunLimitPosition : AdminPageBase
    {
        public string funId = "";
        ...
        {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.AppendFormat("SELECT _AutoID DeptId, DeptName, DeptWBS ,DeptPWBS ,(OrderId+100) OrderId,'' funid,'' Limit FROM T_E_Org_Department where _IsDel=0\r\n                    union select p._AutoID, p.PositionName, p._AutoID ,d.DeptWBS ,p.OrderId,m.funid,m.Limit \r\n                     FROM T_E_Org_Department AS d inner join T_E_Org_Position p on d._AutoID=p.DeptID\r\n                     LEFT OUTER JOIN T_E_Org_PositionLimit AS m ON p._AutoID = m.positionId \r\n                     and (m.FunID = '{0}') ", string_1);
            return SysDatabase.ExecuteTable(stringBuilder.ToString());
        }

        protected void Page_Load(object sender, EventArgs e)
        {
            AjaxPro.Utility.RegisterTypeForAjax(typeof(FunLimitPosition));
        ...
        public string SaveLimit(string funId, string limitxml)
        {
            ...
            try
            {
                ...
                foreach (XmlNode xmlNodes in xmlDocument.SelectNodes("//tn[@EditFlag='1']"))
                {
                    ...
                    {
                        strArrays = new string[] { "update dbo.T_E_Org_PositionLimit set Limit ='", value, "',funwbs='", funWbsById, "' where funid='", funId, "' and positionId='", value1, "'" };
