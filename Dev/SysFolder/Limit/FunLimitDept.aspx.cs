...
{
    public partial class FunLimitDept : AdminPageBase
    {
        ...
        {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.AppendFormat("SELECT d._AutoID DeptId, d.DeptName, d.DeptWBS ,d.DeptPWBS ,d.OrderId,isnull(m.FunID,'') FunID,isnull(m.Limit,'') Limit \r\n                 From T_E_Org_Department AS d LEFT OUTER JOIN T_E_Org_DeptLimit AS m ON d._AutoID = m.DeptID \r\n                 and m.FunID = '{0}' where d._isDel=0 ", string_1);
            return SysDatabase.ExecuteTable(stringBuilder.ToString());
        }

        protected void Page_Load(object sender, EventArgs e)
        {
            AjaxPro.Utility.RegisterTypeForAjax(typeof(FunLimitDept));
        ...
        public string SaveLimit(string funId, string limitxml)
        {
            ...
            try
            {
                ...
                foreach (XmlNode xmlNodes in xmlDocument.SelectNodes("//tn[@EditFlag='1']"))
                {
                    ...
                    {
                        strArrays = new string[] { "update dbo.T_E_Org_DeptLimit set Limit ='", value, "',funwbs='", funWbsById, "' where funid='", funId, "' and DeptID='", value1, "'" };
