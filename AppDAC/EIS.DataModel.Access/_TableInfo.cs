...
namespace EIS.DataModel.Access
{
	public class _TableInfo
	{
		...
		public _TableInfo(string tblName)
		{
			this.string_0 = tblName;
		}

		public _TableInfo(string tblName, DbTransaction dbTran)
		{
			this.string_0 = tblName;
		...
		public _TableInfo(DbTransaction tran)
		{
		...
		{
			return (new _FieldInfo(this.dbTransaction_0)).GetTableFields(this.string_0);
		}
		...
		public TableInfo GetModel()
		{
			...
			else
			{
                ...
                //显示字段的预期提示信息
                model.IsShowFldPlaceholder = false;
                try
                {
                    //显示字段的预期提示信息
                    StringBuilder stringBuilderExt = new StringBuilder();
                    stringBuilderExt.Append("select  top 1 * from T_E_Sys_TableInfoExt ");
                    stringBuilderExt.Append(" where TableName=@TableName ");
                    DbCommand sqlStringCommandExt = SysDatabase.GetSqlStringCommand(stringBuilderExt.ToString());
                    SysDatabase.AddInParameter(sqlStringCommandExt, "@TableName", DbType.String, this.string_0);
                    DataTable dataTableExt = (this.dbTransaction_0 != null ? SysDatabase.ExecuteTable(sqlStringCommandExt, this.dbTransaction_0) : SysDatabase.ExecuteTable(sqlStringCommandExt));
                    if (dataTableExt.Rows.Count > 0)
                    {
                        model.IsShowFldPlaceholder = (dataTableExt.Rows[0]["IsShowFldPlaceholder"].ToString() == "是");
                    }
                }
                catch { }
                finally { }
		...
		{
			return (new _FieldInfo()).GetTablePhyFields(this.string_0);
		}
		...
		{
			return _TableStyle.GetModel(this.string_0, sindex);
		}
		...
		public string GetTableStyle(string sindex, int htmlType)
		{
			...
			try
			{
				if (htmlType == 0)
				{
					formHtml = _TableStyle.GetModel(this.string_0, sindex).FormHtml;
				}
				else if (htmlType == 1)
				{
					formHtml = _TableStyle.GetModel(this.string_0, sindex).FormHtml2;
				}
				else if (htmlType != 2)
				{
					formHtml = (htmlType != 3 ? _TableStyle.GetModel(this.string_0, sindex).FormHtml : _TableStyle.GetModel(this.string_0, sindex).DetailHtml);
				}
				else
				{
					formHtml = _TableStyle.GetModel(this.string_0, sindex).PrintHtml;
				}
			}
